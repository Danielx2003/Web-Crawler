...
function getPlayerLinks(htmlBody, baseURL) {
    ...
    const playerElements = dom.window.document.getElementsByClassName('data-link') //gets the table elements
    const playerURLs = []
    for (const player of playerElements) {
        playerURLs.push(`${baseURL}${player.getAttribute('data-link')}`);
    }
    return playerURLs
}
